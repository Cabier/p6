...
exports.signup = (req, res, next) => {// création de nouveau users dans la base de données à partir de la connection de l'incription depuis l'application front end
bcrypt.hash(req.body.password,10)// va crypter le mot de passe
.then(hash => {
    const user = new User ({
    ...
    user.save()// pour l'enregistrer dans notre base de données
...
exports.login = (req, res, next) => {
    ...
    .then(user =>{
        ...
        bcrypt.compare(req.body.password,user.password)// on compare le mot de passe entrée avec le hash qui est gardé dans la base de données
        .then(valid => {
         if (!valid) {
             return res.status(401).json({error:'Mot de passe incorrect !'})
         } 
         res.status(200).json({// si c'est valable on lui renvoi un user id et un token
             userId: user._id,
             token: jwt.sign(
